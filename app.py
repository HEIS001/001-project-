...
def staff_join():
    ...
    if not email or "@gmail.com" not in email:
        return jsonify({"error": "A valid Gmail address is required to register."}), 400
    with get_db() as conn:
        ...
        if existing:
            if existing["email"] and existing["email"].lower() != email:
                return jsonify({"error": "This name is already registered with a different Gmail."}), 403
        else:
            email_used = conn.execute(
                "SELECT id FROM staff WHERE company_id=? AND email=?",
                (company["id"], email)).fetchone()
            if email_used:
                return jsonify({"error": "This Gmail is already registered by another staff member."}), 409
...
def attendance_register():
    ...
    if registered and registered["email"]:
        if not staff_email:
            return jsonify({"error": "Please enter your registered Gmail to sign in."}), 403
        if registered["email"].lower() != staff_email:
